...
\section{Amplificatori operazionali LF353, LF356 e uA741}
	...
	L'UA741 viene impiegato come filtro attivo passa alto del primo ordine $(e come amplificatore finale)$ per togliere le componenti inferiori a 10Hz come richiesto dal progetto e sfruttare contemporaneamente la banda passante ridotta del dispositvo al fine di attenuare le alte frequenze indesiderate, che verranno tagliate definitvamente dal filtro passa basso.
...
\section{VCO}
	...
	\noindent La frequenza di oscillazione $f_0$ viene controllata da una capacità esterna C, detta capacità di $timing$ collegata tra i \textit{pin5} e $pin6$ e dalla resistenza R posta in ingresso ai $pin7$ e $pin8$. La frequenza di oscillazione si calcola come:
	...
	\noindent Dato che la capacità risulta variabile, in quanto dipende dalla mano dell'utente e dall'antenna realizzata, si è scelta una R di $100k\Omega$ (da verificare) per cercare di rispettare, almeno in parte, il range fornito dal datasheet per garantire una buona stabilità in temperatura. 
...
\section{Filtro LP del quarto ordine}
	...
	\begin{equation}
		H_1(s) = \frac{V_{in}}{V_u}  =  \frac{1}{1 + s(C_1R_1 + C_2R_2)+ s^2C_1C_2R_1R_2 } 
	\end{equation}

	Mentre la funzione di trasferimento della seconda cella risulta essere:

	\begin{equation}
		H_2(s) = \frac{V_u}{V_{out}}  = \frac{1}{1 + s(C_3R_3 + C_4R_4)+ s^2C_3C_4R_3R_4 } 
	\end{equation}
